...
The analysis uses real **CMS Open Data** stored in ROOT format.

Two CMS event samples containing approximately **1 million collision events in total** were analyzed. The ROOT files contain reconstructed physics objects including muons and missing transverse momentum.

The raw ROOT files are not included in the repository because of their large size.
...
The muons are converted into four-vectors using their transverse momentum \(p_T\), pseudorapidity \(\eta\), azimuthal angle \(\phi\), and mass.
...
The dimuon invariant-mass spectrum was first investigated around the J/ψ mass.
...
The same dimuon invariant-mass technique was used to investigate the Υ resonance.

The Υ(1S) mass is approximately

\[
m_{\Upsilon(1S)}\approx9.46\text{ GeV}.
\]

The dimuon spectrum was examined around this region to search for the corresponding resonance signature.

![Υ peak](fig/result/upsilon_peak_clean.png)

The J/ψ and Υ analyses use the same underlying principle:

\[
\boxed{
\text{two visible decay products}
\rightarrow
\text{invariant mass}
\rightarrow
\text{resonance}
...
The same dimuon invariant-mass reconstruction was then used to study the Z boson.
...
is particularly suitable for invariant-mass reconstruction because both decay products are visible.

Tighter muon selections were applied using:
...
\[
m_Z\approx91.2\text{ GeV}.
\]

![Z fit](fig/result/z_fit.png)
...
The muon can be reconstructed by the detector, while the neutrino does not leave a directly measurable track.

Instead, the neutrino's transverse momentum is inferred from the event's missing transverse momentum.

The missing transverse momentum is represented by

\[
\vec{p}_T^{\,miss}.
\]

The CMS data provides its magnitude and direction through `MET_pt` and `MET_phi`.

For a W → μν event, the missing transverse momentum can be interpreted as the neutrino's transverse-momentum footprint:
...
Because the neutrino's complete three-dimensional momentum is unknown, the ordinary invariant mass cannot be reconstructed in the same way as for the Z boson.

Instead, the transverse mass is calculated using
...
with the difference restricted to

\[
0\leq\Delta\phi\leq\pi.
\]
...
\[
m_W\approx80.4\text{ GeV}.
\]

![W transverse mass](fig/result/transverse_mass.png)
...
both decay products are visible, so the invariant mass can be reconstructed directly.
...
the neutrino is invisible, so missing transverse momentum and transverse mass are used instead.

![W and Z analysis](fig/result/w_z_analysis.png)
...
The neutrino cannot be directly plotted as a reconstructed detector object because its momentum is not fully measured.

However, its transverse momentum footprint can be visualized using the missing transverse momentum.

For a selected W candidate event, the muon and MET are converted into Cartesian transverse momentum components:
...
![Neutrino event display](fig/result/neutrino_event_display.png)

The detected muon is represented by its measured transverse momentum vector.

The missing transverse momentum vector represents the momentum imbalance in the event and is interpreted as the **neutrino candidate's transverse momentum** in the W → μν topology.

The visualization therefore shows the difference between a directly reconstructed particle and an invisible particle inferred from missing momentum.
...
These are studied through the dimuon invariant mass:
